...
namespace DrivingSimulation
{
    class DebugCrossroadsRoadGraph : BuildableRoadGraph
    {
        ...
        {
            var C = Wr.CrossroadsX(new Vector2(8, 8), 0, 3);
    ...
    class CrossroadsTRoadGraph : BuildableRoadGraph
    {
        ...
        {
            var C = Wr.CrossroadsT(new Vector2(8, 2), 0, new Vector2(3), new List<Func<SimulationObjectCollection, RoadPlugView>>() { x => x.DefaultLeft1Side(true).GetView(false), x => x.DefaultBottom2Side().GetView(false), x => x.DefaultRight1Side().GetView(false) });
    ...
    class QuadCrossroadsRoadGraph : BuildableRoadGraph
    {
        public QuadCrossroadsRoadGraph(RoadWorld world) : base(world, new Vector2(26, 26))
        {
            ...
            var A = Wr.CrossroadsXMainLR(new Vector2(8, 8), 0, 3);
            var B = Wr.CrossroadsXMainLR(new Vector2(18, 8), 0, 3);
            var C = Wr.CrossroadsXMainLR(new Vector2(8, 18), 0, 3);
            var D = Wr.CrossroadsXMainLR(new Vector2(18, 18), 0, 3);

            foreach (RoadPlugView p in new RoadPlugView[] { A.T, B.T, A.L, C.L, C.B, D.B, B.R, D.R })
            {
                GarageRoad(p, g.Copy());
            }
            Wr.Connect(A.B, C.T); Wr.Connect(B.B, D.T); Wr.Connect(A.R, B.L); Wr.Connect(C.R, D.L);
        }
    }
    ...
    class PankracWorldGraph : BuildableRoadGraph
    {
        ...
        public PankracWorldGraph(RoadWorld world) : base(world, new Vector2(120, 120))
        {
            ...
            CameraZTo = 80;
            CameraZoomSpeed = 1;
            PathRandomizationFrom = 0.5f;
            PathRandomizationTo = 3;
            RecommendedVehicleCount = 600;
            ...
            var DO_DE = CrossroadsT(DEF2, DEF2, DEF2, 95.8, 9.2, 150); Wr.Connect(DO_D.R, DO_DE.R); GarageRoad(DO_DE.B, new PeriodicBurstGarage(.75f, 6, 4, new LoopColorPicker()));
            ...
            var DO_IJ = CrossroadsT(DEF2, DEF2, DEF2, 109.1, 92.3, 50); Wr.Connect(DO_I.R, DO_IJ.R); GarageRoad(DO_IJ.B, new PeriodicBurstGarage(.75f, 6, 4, new LoopColorPicker()));
            ...
            var DO_M = CrossroadsX(DEF2, DEF2, INV1, DEF1, 21.0, 81.9, 135);  Wr.Connect(DO_L.R, DO_M.L); GarageRoad(DO_M.T, new PeriodicBurstGarage(.75f, 6, 10, new LoopColorPicker()), 0.5f);
            ...
            var UDI_AC =CrossroadsT(INV1, DEF2, DEF1, 44.6, 70.6, 233); Wr.Connect(UDI_A.B, UDI_AC.R); GarageRoad(UDI_AC.B, new PeriodicBurstGarage(.75f, 6, 4, new LoopColorPicker()));
            ...
            var UDZ_B = CrossroadsX(DEF2, INV1, DEF1, DEF1, 59.4, 79.2, 53); Wr.Connect(UDZ_B.L, UDZ_A.L); GarageRoad(UDZ_B.T, new PeriodicBurstGarage(.75f, 6, 10, new LoopColorPicker()), 0.5f);
